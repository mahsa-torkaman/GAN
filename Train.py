...
real_img_gt = tf.concat([img, gt, mask], 3)
fake_syn_gt = tf.concat([syn, gt, mask], 3)
...
if style_flag == 'drive':
    #pdb.set_trace()
    ss = sio.loadmat('trn20_tst20_double2.mat')['M_train_images'][styleNum]
...
# write summary
#Dx_sum = tf.histogram_summary("Dx", Dx) API has changed (Mahsa)
...
syn_sum = tf.summary.image("synthesize", syn)
...
g_sum = tf.summary.merge([Dg_sum_scalar])
d_sum = tf.summary.merge([Dx_sum_scalar])

# =============================== train phase ============================================= #
#init = tf.initialize_all_variables() M:Deprecated
...
for epoch in xrange(max_epoch):
    ...
    #for i in range(20):
            ...
            #pdb.set_trace()
            #test_1To4:M_images,M_masks,M_manual1
            ss = sio.loadmat('test_1To4_double2.mat')['M_images'][3]
            ss = (np.reshape(ss, [batch_size, img_size, img_size, 3]) - 0.5) * 2.0
            ss = np.lib.pad(ss, ((0, 0), (padding_l, padding_r), (padding_t, padding_d), (0, 0)), 'constant',
                            constant_values=(-1, -1))  # Pad the images so the are 32x32
            ...
            if training_util.global_step(sess, global_step) % 100 == 0:
                ...
                img_sample = (np.reshape(img_sample[0:4], [sample_batch, img_x, img_y, channel]) - 0.5) * 2.0
                # Pad the images so the are 32x32
                img_sample = np.lib.pad(img_sample, ((0, 0), (padding_l, padding_r), (padding_t, padding_d), (0, 0)),
                                        'constant', constant_values=(-1, -1))

                gt_sample = (np.reshape(gt_sample[0:4], [sample_batch, img_x, img_y, 1]) - 0.5) * 2.0
                # Pad the images so the are 32x32
                gt_sample = np.lib.pad(gt_sample, ((0, 0), (padding_l, padding_r), (padding_t, padding_d), (0, 0)),
                                       'constant', constant_values=(-1, -1))

                mask_sample = (np.reshape(mask_sample[0:4], [sample_batch, img_x, img_y, 1]) - 0.5) * 2.0
                # Pad the images so the are 32x32
                ...
                ###########################################Mahsa
                #mask_sample=np.array(mask_sample, dtype=bool)#Mahsa
                Msa = np.array(((mask_sample[sa] + 1) / 2),dtype=bool)
                Msb = np.array(((mask_sample[sb] + 1) / 2),dtype=bool)
                Msc = np.array(((mask_sample[sc] + 1) / 2),dtype=bool)
                Msd = np.array(((mask_sample[sd] + 1) / 2),dtype=bool)
                syn_sample_am = syn_sample_a * Msa #((mask_sample[sa] + 1) / 2)
                syn_sample_bm = syn_sample_b * Msb #((mask_sample[sb] + 1) / 2)
                syn_sample_cm = syn_sample_c * Msc #((mask_sample[sc] + 1) / 2)
                syn_sample_dm = syn_sample_d * Msd #((mask_sample[sd] + 1) / 2)
