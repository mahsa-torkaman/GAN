...
class DataBlocks(SimpleBlocks):
    ...
    def load_file(self, paths, queue=None):
        ...
        train = []
        valid = []
        temp = [] #Mahsa (To make the output size right)
        i=1 #Mahsa
        if isinstance(paths, (list, tuple)):
            for path in paths:
                ...
                if (i==1):#Mahsa
                    d1=np.array(train).reshape(1,512,512,3)#Mahsa
                    temp.append(d1)#Mahsa
                else:#Mahsa
                    d2=np.array(train[1]).reshape(1,512,512)#Mahsa
                    temp.append(d2)#Mahsa
